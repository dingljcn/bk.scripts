...
export default {
    template: `<div id="ticket-list-filter">
        <div class="filter-row">
            <inputx class="filter-contrl" @on-input="data => { filter.keyword = data.value; $emit('on-change', filter) }" 
                @on-change="data => { filter.keyword = data.value; $emit('on-change', filter) }" 
                caption="搜索" 
                placeholder="请输入关键字">
            </inputx>
