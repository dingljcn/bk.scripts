...
/** 获取配置, 如果取不到则返回默认值 */
defunc(window.dinglj, 'getVal', (data, path, _default) => {
    return dinglj.getVal(data, path, _default, false);
});
...
defunc(window.dinglj, 'getConfigOrDefault', (config, defaultConfig, path, _default, merge) => {
    let config1 = dinglj.getVal(config, path, 'NOT_FOUND');
    let config2 = dinglj.getVal(defaultConfig, path, 'NOT_FOUND');
...
defunc(window.dinglj, 'injectUserCss', () => {
    ...
    const css = dinglj.getVal(config, 'css', '');
    let styleElement = document.createElement('style');
