const TipUtils = {
    distoryPopMsg(element: Element, container: HTMLElement) {
        if (!element) {
            return;
        }
        element.animate({
    ...
    createPopMsg(msg: string, type: 'info' | 'warn' | 'err', timeout = 2000, marginTop = '10%') {
        // 创建容器
        ...
        if (msgList.length >= 5) {
            ...
            for (let i = 0; i < msgList.length - 4; i++) {
                this.distoryPopMsg(msgList[i], undefined);
            }
        }
        // 创建一条提示信息
        ...
        // 一定时间后销毁该消息
        setTimeout(() => {
            this.distoryPopMsg(newElemenet, container);
        }, timeout);
    },
    info(msg: string, timeout = 2000, offsetTop = '10%'): void {
        this.createPopMsg(msg, 'info', timeout, offsetTop);
    },
    warn(msg: string, timeout = 2000, offsetTop = '10%'): void {
        this.createPopMsg(msg, 'warn', timeout, offsetTop);
    },
    err(msg: string, timeout = 2000, offsetTop = '10%'): void {
        this.createPopMsg(msg, 'err', timeout, offsetTop);
    }
}
